...
const ParseString = (string, { scale }) => { 
    ...
    for (const char of seperated) {
        if (char === " ") {
            distance = distance+(0.8*(scale || 0.25));
            continue;
        }
        ...
        modifiedLetter.xOffset += distance;
        output.push(modifiedLetter);
        distance += (largestX * modifiedLetter.scaleFactor) + (0.2 * modifiedLetter.scaleFactor);
    }
